...
int Jugador::buscarRegistro(const char* dniBuscado){
    Jugador obj;
    FILE *p = fopen("jugadores.dat", "rb");
    if(p == nullptr){
        return -1;
    ...
    while (fread(&obj, sizeof(Jugador), 1, p) == 1) {
        if (strcmp(obj.getDNI(), dniBuscado) == 0) {
            fclose(p);
            return pos; // encontrado en esta posición
        }
        pos++;
    }
    fclose(p);
...
void Jugador::buscarPorDNI(const char* dni) {
    int cantReg = contarRegistros();
    for (int i = 0; i < cantReg; i++) {
        Jugador obj = leerRegistro(i);
        if (strcmp(obj.getDNI(), dni) == 0) {
            obj.mostrar();
            return;
        }
    }
    cout << "No se encontró un jugador con ese DNI." << endl;
}
